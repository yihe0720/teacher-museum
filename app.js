...
import {setupAdmission} from './admission.js';
import {createVisit,setupSouvenir} from './souvenir.js';
...
function recordArrival(){if(admitted)visit.arrive(room,targetId);}
...
function sync(){const r=museum.rooms[room],e=exhibit(targetId);$('room-kicker').textContent=roman[room]+' / '+r.english;$('room-title').textContent=r.title;
 $('ending-invitation').hidden=room!==5;document.body.classList.toggle('ending-room',room===5);
 $('room-desc').textContent=e?e.title:room===0?museum.rooms[0].entry:room===5?'教师节快乐，何老师。':r.entry?'在入口，留了几句话。':'可以从任何一幅照片开始。';
 $('read-item').hidden=targetId==='exit'||targetId==='back'||(!e&&room===1);
 $('read-item').innerHTML=(e?.kind==='drawer'?(current.targets.get('drawer').opened?'收回抽屉':'拉开抽屉'):e?'读展签':room===0?'读序言':room===5?'给您的话':'读入口题记')+' <span>↗</span>';
 $('drawer-read').hidden=!(e?.kind==='drawer'&&current.targets.get('drawer').opened);
 $('book-actions').hidden=e?.kind!=='books';$('book-actions').replaceChildren();
 if(e?.kind==='books')e.books.forEach((title,i)=>{const b=document.createElement('button'),opened=current.targets.get('books').books[i].userData.opened;b.textContent=(opened?'放回':'抽出')+'《'+title+'》';b.setAttribute('aria-pressed',String(opened));b.onclick=()=>toggleBook(i);$('book-actions').append(b);});
...
async function setRoom(i,id='entry',animate=true){
 ...
 try{current=getRoom(i);room=i;if(i===5){document.body.classList.remove('quiet');$('exit-quiet').hidden=true;}targetId=current.targets.has(id)?id:'entry';const target=current.targets.get(targetId);camera.position.copy(target.position);const o=orientation(target.position,target.look);yaw=aimYaw=o.yaw;pitch=aimPitch=o.pitch;enteredAt=focusedAt=performance.now();endingReady=false;recordArrival();sync();}
...
function moveTo(id){
 ...
 if(reduced||!path){camera.position.copy(t.position);yaw=aimYaw=o.yaw;pitch=aimPitch=o.pitch;motion=null;recordArrival();}
...
function openDetail(id){
 ...
 const e=exhibit(id),r=museum.rooms[room];if(e)visit.read(id);let title='',text='',date='',noVisual=false;
 ...
 $('detail-dialog').classList.toggle('text-only',noVisual);$('detail-kicker').textContent=r.english+' / '+r.title;$('detail-title').textContent=title;$('detail-date').textContent=date;paragraphs($('detail-text'),text);$('detail-location').textContent=r.name+' · '+r.title;$('detail-next').hidden=room===5;$('souvenir-detail-open').hidden=room!==5;showDialog('detail-dialog');
 if(e&&(e.kind==='object'||e.kind==='trophy'))requestAnimationFrame(()=>makeViewer(e.id));
}
function startSlide(object,to){object.userData.slide={from:object.position.z,to,start:performance.now()};}
function toggleBook(index){if(motion||switching||room!==2)return;const b=current.targets.get('books')?.books[index];if(!b)return;b.userData.opened=!b.userData.opened;if(b.userData.opened)visit.moment('book:'+index);startSlide(b,b.userData.opened?.60:.085);sync();status((b.userData.opened?'已抽出':'已放回')+exhibit('books').books[index]);}
function readCurrent(){
 if(motion||switching)return;
 const e=exhibit(targetId);
 if(e?.kind==='drawer'){const t=current.targets.get('drawer');t.opened=!t.opened;if(t.opened)visit.moment('drawer');startSlide(t.drawer,t.opened?.77:.14);sync();status(t.opened?'抽屉已拉开，可以阅读里面的资料。':'抽屉已收回。');return;}
 openDetail(targetId);
}
...
function setupUI(){
 setupSouvenir(visit);document.addEventListener('visibilitychange',()=>visit.visibility(!document.hidden));
 ...
 $('home').onclick=e=>{e.preventDefault();guided=false;setRoom(0);};$('map-open').onclick=$('map-open-bottom').onclick=()=>showDialog('map-dialog');$('help').onclick=()=>showDialog('help-dialog');$('read-item').onclick=readCurrent;$('drawer-read').onclick=()=>{if(current.targets.get('drawer')?.opened)openDetail('drawer');};
...
function updateAnimations(time){
 ...
 const sliders=[current.targets.get('drawer')?.drawer,...(current.targets.get('books')?.books||[])].filter(Boolean);
 for(const object of sliders){const a=object.userData.slide;if(!a)continue;const t=reduced?1:T.MathUtils.clamp((time-a.start)/700,0,1);object.position.z=a.from+(a.to-a.from)*t*t*(3-2*t);if(t===1)delete object.userData.slide;}
...
function animate(time){requestAnimationFrame(animate);if(document.hidden||!current)return;const dt=Math.min(.05,(time-last)/1000||.016);last=time;
 if(motion){const m=motion,t=Math.min(1,(time-m.start)/m.duration),k=t*t*(3-2*t);let dist=m.total*k,i=0;while(i<m.lengths.length-1&&dist>m.lengths[i]){dist-=m.lengths[i];i++;}camera.position.lerpVectors(m.points[i],m.points[i+1],m.lengths[i]>0?Math.min(1,dist/m.lengths[i]):1);yaw=aimYaw=m.fromYaw+(m.toYaw-m.fromYaw)*k;pitch=aimPitch=m.fromPitch+(m.toPitch-m.fromPitch)*k;if(t>=1){camera.position.copy(m.points.at(-1));motion=null;focusedAt=time;recordArrival();}}
...
async function init(){
 try{
  ...
  await Promise.all(names.map(name=>new Promise(resolve=>{const img=new Image();img.onload=()=>{images.set(name,img);$('loading-progress').textContent=++loaded+' / '+names.length;admission.setProgress(loaded,names.length);resolve();};img.onerror=()=>{failures.push(name);resolve();};img.src=asset(name);})));if(failures.length)throw new Error('缺少素材：'+failures.join('、'));
  await document.fonts.ready;await setRoom(0,'prologue',false);requestAnimationFrame(animate);$('loading').classList.add('ready');admission.setReady();window.addEventListener('resize',resize);
 ...
 }catch(error){console.error(error);$('loading-note').textContent=error.message?.startsWith('缺少素材')?'部分照片没有载入，请重新载入。':'三维展馆暂时无法打开，请使用支持 WebGL 的浏览器。';$('loading-progress').textContent='';$('retry').hidden=false;admission.setError($('loading-note').textContent);}
}
admission=setupAdmission({onEnter:()=>{admitted=true;visit.start();recordArrival();enteredAt=focusedAt=performance.now();},onStatus:status});
init();
